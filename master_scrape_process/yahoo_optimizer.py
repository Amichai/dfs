...
def optimize_roster(roster, by_position):
    ...
    if initial_cost <= 200:
        ...
        while True:
            ...
            replacement = select_better_player(by_position[position], roster.remainingFunds(200) + to_swap.cost, excluding, to_swap.value)
...
def random_optimizer(by_position):
    ...
    for i in range(500000):
        ...
        for pos, players in by_position.items():
            if to_remove != None and to_remove.name in [p.name for p in players]:
                players_new = [p for p in players if p.name != to_remove.name]
        ...
        if random_lineup.cost > 200 or not random_lineup.is_valid:
            continue
